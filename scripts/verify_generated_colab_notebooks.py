        ...
        for generated in sorted(temporary_notebooks.rglob("*.ipynb")):
            ...
            try:
                ...
                if capability == "subtitle-ocr":
                    ...
                    if worker_source.count('ocr_pip("--no-cache-dir"') != 4 \
                            or 'ocr_pip("install"' in worker_source \
                            or '"langchain==0.3.27"' not in worker_source \
                    ...
                    if 'langchain==0.2.17' in worker_source:
                        mismatches.append(
                            f"Subtitle OCR notebook must not install the incompatible LangChain 0.2 stack: {generated.name}"
                        )
                    if 'from langchain.docstore.document import Document' not in worker_source:
                        mismatches.append(
                            f"Subtitle OCR notebook must prove the PaddleX-required LangChain docstore import: {generated.name}"
                        )
