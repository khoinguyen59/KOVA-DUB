                                           ...
                                           const QString &referencePresetId, float speed, int steps)
{
    ...
    m_activeVoiceName = request.referenceName;
                                                  ...
                                                  int sampleRate)
{
    ...
    emit outputChanged();
    if (m_history) {
        m_history->addTtsHistorySamples(
            m_activeText, QStringLiteral("Colab GPU voice clone (%1)").arg(m_model),
            m_activeVoiceName.isEmpty() ? QStringLiteral("Clone") : m_activeVoiceName,
