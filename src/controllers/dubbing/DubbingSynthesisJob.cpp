...
#include "tts/runners/ColabTtsRunner.h"
#include "tts/runners/ColabVoiceCloneRunner.h"
#include "tts/runners/GatewayTtsRunner.h"
#include "tts/engine/TtsEngine.h"
#include "tts/pipeline/TtsSavedVoiceProfile.h"
#include "workflows/session/WorkflowArtifact.h"
                                ...
                                const QVariantMap &settings, const QString &runId)
{
    ...
    const QVariantMap savedClonePreset = settings.value(
        QStringLiteral("savedTtsVoicePreset")).toMap();
    // A key with an empty map must not turn a normal TTS run into clone mode.
    // This protects the controller/UI boundary when a user changes from a
    // saved voice to a built-in voice before starting the next task.
    const bool hasSavedClonePreset = !savedClonePreset.isEmpty()
        && !savedClonePreset.value(QStringLiteral("id")).toString().trimmed().isEmpty();
    // Legacy settings are retained only for already-saved pre-library
    // projects. A valid modern selection is authoritative: an old
    // voiceCloningEnabled/cloneVoicePreset pair must never replace the voice
    // the user selected in the reusable TTS library.
    m_legacyCloneSettings = !hasSavedClonePreset
        && settings.value(QStringLiteral("voiceCloningEnabled")).toBool();
    m_useVoiceCloning = hasSavedClonePreset || m_legacyCloneSettings;
    m_voiceReference = {};
    ...
    if (remote) {
        ...
        if (m_executionProvider == ExecutionProvider::ApiGateway && m_gatewaySettings) {
            ...
            if (m_useVoiceCloning) {
                // Permission is a user-facing prerequisite, not a value the
                // Direct Colab worker may invent. Reject before profile
                // creation or reuse so a stale project cannot clone a voice
                // without a current explicit confirmation. Keep the API
                // Gateway branch above independent: it must still report its
                // own unsupported-route error rather than a Direct Colab one.
                if (!settings.value(QStringLiteral("voiceCloneConsentConfirmed")).toBool()) {
                ...
                if (!DubbingColabModelRoutes::supports(
                        QStringLiteral("voice-cloning"), cloneModel)) {
                    fail(QStringLiteral("No exact Direct Colab Voice Cloning notebook is mapped for '%1'.")
                             .arg(cloneModel));
                    return false;
                }
                QString routeError;
                ...
                m_cacheSettings.insert(QStringLiteral("effectiveVoiceCloneModel"),
                                       cloneModel);
                // The clone worker receives its own exact model; the ordinary
                // TTS selection is neither validated nor substituted here.
                effectiveModel = cloneModel;
                voice = QStringLiteral("saved-clone");
            } else {
        ...
        m_cacheSettings.insert(QStringLiteral("effectiveRemoteModel"), effectiveModel);
        m_cacheSettings.insert(QStringLiteral("effectiveRemoteVoice"), voice);
        m_synthesisSignature = QStringLiteral("dubbing-tts|%1|%2|%3|%4")
            .arg(executionProviderId(m_executionProvider), effectiveModel, voice,
    ...
    if (m_useVoiceCloning) {
        ...
        if (m_executionProvider == ExecutionProvider::ColabDirect) {
            ...
            const QString profileSignature = QStringLiteral("%1|%2|%3|%4|%5|%6|%7")
                .arg(m_cloneVoicePresetId,
                                                ...
                                                quint64 requestId)
{
    ...
    request.consentConfirmed = requestSettings.value(
        QStringLiteral("voiceCloneConsentConfirmed")).toBool();
