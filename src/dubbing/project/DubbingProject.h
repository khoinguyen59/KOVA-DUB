...
// Generated bytes remain in the project's cache directory, while their paths
// and the workflow checkpoint are persisted so a reopened project can resume
// without forcing the user to repeat completed work.
class DubbingProject
{
public:
    // Schema 17 stores assets that live below the project folder as relative
    // paths.  Absolute paths from legacy projects remain readable so a normal
    // save can migrate them without losing links to external user media.
    ...
    // Durable workflow checkpoint. These fields contain only local state and
    // output metadata; credentials and transient worker handles never belong
    // in a project file.
    QString workflowCurrentStepId;
    QString workflowLastCompletedStepId;
    QVariantMap workflowStepOutputs;
    QString previewAudioPath;
    QString dubbedVocalAudioPath;
    QString exportMediaPath;
    QString capCutDraftPath;
