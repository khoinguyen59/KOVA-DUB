...
void TestColabSeparationRunner::separationNotebookMatchesDirectColabContract()
{
    ...
    for (const Expectation &expected : expectations) {
        ...
        const QString path = resolveNotebookFile(QDir(QStringLiteral(LASTUDIO_SOURCE_DIR)), expected.notebook);
        QFile file(path);
        ...
        if (expected.model == QStringLiteral("sherpa-onnx-spleeter-2stems-fp16")) {
            ...
            QVERIFY2(source.contains(QStringLiteral("and worker.poll() is None"))
                         && source.contains(QStringLiteral("and tunnel.poll() is None")),
                     "The launcher must never publish a candidate URL after the CUDA worker or Cloudflare process has exited.");
        }
