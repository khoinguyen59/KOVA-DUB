...
#include "stt/runners/ColabSttRunner.h"
#include "stt/runners/GatewaySttRunner.h"
...
void TestSttSession::testSpeechNotebookMatchesDirectColabSttContract()
{
    ...
    for (const NotebookExpectation &expected : expectations) {
        const QString path = resolveNotebookFile(QDir(QStringLiteral(LASTUDIO_SOURCE_DIR)), expected.fileName);
        QFile file(path);
