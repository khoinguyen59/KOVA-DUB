...
void TestRemoteExecution::everyGpuFeatureSurfacesVerifiedColabSessionState()
{
    ...
    QFile sessionSource(sourceRoot.filePath(QStringLiteral("src/remote/colab/ColabSession.cpp")));
...
void TestRemoteExecution::voiceCloneOmniVoiceIsReusableInTtsWithoutLocalFallback()
{
    ...
    QVERIFY(referenceSource.contains(QStringLiteral("function reloadSavedVoices(clearSelection)")));
    QVERIFY(referenceSource.contains(QStringLiteral("root.reloadSavedVoices(false)")));
    QVERIFY(referenceSource.contains(QStringLiteral("root.selectedSavedVoiceId = previousId")));
    ...
    QVERIFY(cloneSource.contains(QStringLiteral("pendingReusableVoiceFamilyId")));
    QVERIFY(cloneSource.contains(QStringLiteral("activeCloneModelId")));
    QVERIFY(cloneSource.contains(QStringLiteral("activeCloneFamilyId")));
    QVERIFY(cloneSource.contains(QStringLiteral("onSynthesisFinished")));
    QVERIFY(cloneSource.contains(QStringLiteral("voiceClonePresets.addPreset(familyId")));
    QVERIFY(cloneSource.contains(QStringLiteral("familyId: root.activeCloneFamilyId")));
    QVERIFY(cloneSource.contains(QStringLiteral("Enter a Voice name for TTS reuse")));

    QFile cloneController(sourceRoot.filePath(QStringLiteral("src/controllers/tts/ColabVoiceCloneController.cpp")));
    QVERIFY(cloneController.open(QIODevice::ReadOnly));
    const QString cloneControllerSource = QString::fromUtf8(cloneController.readAll());
    QVERIFY(cloneControllerSource.contains(QStringLiteral("m_activeVoiceName = request.referenceName")));
    QVERIFY(cloneControllerSource.contains(
        QStringLiteral("m_activeVoiceName.isEmpty() ? QStringLiteral(\"Clone\") : m_activeVoiceName")));
    ...
    // A clone worker is a separate exact-model route. TTS must not mutate its
    // ordinary Colab selection merely because a saved clone worker connects.
    ...
    QVERIFY(settings.open(QIODevice::ReadOnly));
    const QString settingsSource = QString::fromUtf8(settings.readAll());
    QVERIFY(settingsSource.contains(QStringLiteral("Reuse cloned voice")));
    QVERIFY(settingsSource.contains(QStringLiteral("reusableCloneVoices")));
    QVERIFY(settingsSource.contains(QStringLiteral("presetsForFamily(root.cloneVoiceModel)")));
    QVERIFY(!settingsSource.contains(QStringLiteral("presetsForFamily(\"omnivoice\")")));
    QVERIFY(settingsSource.contains(QStringLiteral("I have permission to use this cloned voice for TTS")));
    QVERIFY(settingsSource.contains(QStringLiteral("Use cloned voice in TTS")));
    QVERIFY(settingsSource.contains(QStringLiteral("if (previousId !== nextId)")));
    QVERIFY(settingsSource.contains(QStringLiteral("root.reusableCloneConsent = false")));
    QVERIFY(!settingsSource.contains(QStringLiteral("TTS selects OmniVoice automatically")));
    ...
    QVERIFY(ttsSource.contains(QStringLiteral("cloneVoiceActive")));
    QVERIFY(ttsSource.contains(QStringLiteral("cloneVoiceModel")));
    ...
    QVERIFY(ttsSource.contains(QStringLiteral(
        "savedVoice.name || \"LA Studio voice\", settingsPanel.reusableCloneConsent,")));
    QVERIFY(ttsSource.contains(QStringLiteral("Voice Clone Colab")));
    ...
    QVERIFY(ttsSource.contains(QStringLiteral("else if (colabActive && selectedRemoteProvider === \"\")")));
    QVERIFY(!ttsSource.contains(QStringLiteral("else if (colabActive)\n            selectedRemoteProvider = \"colab\"")));
}
...
void TestRemoteExecution::colabNotebooksAdvertiseCapabilityContractVersion()
{
    ...
    for (const NotebookContract &notebook : notebooks) {
        QFile file(resolveNotebookFile(sourceRoot, notebook.file));
