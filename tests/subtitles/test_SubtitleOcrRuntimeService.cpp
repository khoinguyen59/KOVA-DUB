...
static QString readSubtitleOcrControllerSources(const QDir &sourceRoot = QDir(QStringLiteral(LASTUDIO_SOURCE_DIR))) {
    QFile mainFile(sourceRoot.filePath(QStringLiteral("src/controllers/subtitles/SubtitleOcrController.cpp")));
    QString text;
    if (mainFile.open(QIODevice::ReadOnly)) text = QString::fromUtf8(mainFile.readAll());
    const QDir partsDir(sourceRoot.filePath(QStringLiteral("src/controllers/subtitles/parts")));
    if (partsDir.exists()) {
...
void TestSubtitleOcrRuntimeService::qmlRouteRoiAndManagedRuntimeControlsAreWired()
{
    ...
    QFile runtimeService(sourceRoot.filePath(QStringLiteral("src/subtitles/ocr/SubtitleOcrRuntimeService.cpp")));
    ...
    const QString controllerSource = readSubtitleOcrControllerSources(sourceRoot);
