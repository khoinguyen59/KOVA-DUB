...
StudioShell {
    ...
    readonly property string cloneVoiceModel: AppController.colabVoiceClone
    ...
    readonly property var remoteTts: selectedRemoteProvider === "gateway" && gatewayActive
                                 ...
                                 : (selectedRemoteProvider === "colab" && colabActive
                                    ...
                                    : (selectedRemoteProvider === "clone" && cloneVoiceActive
    ...
    readonly property string remoteTtsLabel: selectedRemoteProvider === "gateway"
                                             ...
                                             : (selectedRemoteProvider === "clone"
                                                ? qsTr("%1 Voice Clone Colab").arg(root.cloneVoiceModel)
                                                : qsTr("Colab GPU"))
    ...
    function selectRemoteProvider(provider) {
        ...
        else if (provider === "clone" && cloneVoiceActive)
            selectedRemoteProvider = "clone"
        else if (provider === "")
    ...
    onColabActiveChanged: {
        if (!colabActive && selectedRemoteProvider === "colab")
            selectedRemoteProvider = ""
        // Regular TTS Colab becoming ready must not replace an explicit
        // OmniVoice clone route. Routes are user choices, not readiness
        // side-effects; select Colab automatically only when none is chosen.
        else if (colabActive && selectedRemoteProvider === "")
            selectedRemoteProvider = "colab"
    }
