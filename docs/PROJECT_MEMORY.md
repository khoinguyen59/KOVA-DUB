...
- `DubbingArtifactUploadPanel` khong duoc giu `FileDialog` tinh cho artifact
  Colab. Dung lazy `Loader` + `Component.onCompleted: open()` cho moi picker;
  sau accept chi luu path da chon, con controller van la authority validate
  ten/extension, huy dung transfer cua dung task va advance workflow.
- `FileDialog` khong duoc ghi de binding artifact spec sau khi upload; spec
  phai la readonly binding theo node hien tai de giao dien khong hien contract
  cu khi doi task.

## 2026-08-24 - CapCut-style inline caption editing trong Dubbing

- `subtitleConfiguration` phai co default style luu trong project. Preview
  hien caption ngay khi segment hien tai co text; dung `target` neu duoc chon,
  roi fallback `source`.
- Click caption tren preview mo `DubbingInlineSubtitleEditor` cho dung segment
  va luu qua `dubbing.updateSegment`. Day la luong sua text thong thuong.
- `DubbingSubtitleEditor` lon chi duoc goi tu **Subtitle style & import**.
  Tuyet doi khong tu mo no trong `runNextNode` sau transcript/translation.
- Baseline phat hanh internal moi: `out/LA-Studio-0.0.7.9/LA-Studio-0.0.7.9.exe`
  (SHA-256 `2101E91B16019BC28046E814D571761079DBD2B1444699A7C56E3BC8FC3C27D4`).
  `TestDubbingProject` va QML route smoke da pass; full CTest con mot baseline
  textual assertion cu o `TestColabSeparationRunner`, khong lien quan caption.
...
### 2026-08-24 - Saved clone voices are independent of ordinary TTS workers

- A reusable clone now belongs to the exact `voice-cloning` model that created
  it, not to the currently selected ordinary TTS model. The supported model
  map is maintained in `DubbingColabModelRoutes` and points to actual
  model-specific notebooks.
- Connecting a Voice Cloning Colab worker must never overwrite the normal TTS
  Colab model or force the TTS UI to OmniVoice. TTS selects the clone route
  only after the user selects a saved clone and confirms permission.
- Dubbing persists a selected clone family independently under
  `voiceCloneModelId`; regular TTS `modelId` remains the normal-TTS choice.
  When a built-in voice is selected, clone-only settings are stripped at the
  controller-to-job boundary.
- A modern `savedTtsVoicePreset` takes precedence over legacy
  `voiceCloningEnabled`/`cloneVoicePreset` values. Direct clone synthesis
  requires fresh `voiceCloneConsentConfirmed`; its cache key includes the
