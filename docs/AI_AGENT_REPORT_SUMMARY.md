...
- Sua panel **Upload completed Reviewed STT + OCR transcript** va cac artifact
  ...
  file da chon va cap nhat trang thai de nut **Use uploaded output and
  continue** bat sau khi co file hop le. Khong con binding stale khi import.
- Regression bo sung: current OCR revision phai verify duoc, va QML artifact
  picker phai dung lazy dialog/open-on-click. Sua mot assertion Spleeter stale
  de kiem tra dung thong diep notebook hien hanh.
- Bang chung: build Windows test thanh cong; `TestDubbingProject` 1/1 pass;
  CTest day du 39/39 pass. Khong mo GUI, khong khoi dong Colab live va khong
  dong goi EXE trong dot sua nay.

## 2026-08-24 - Subtitle hien san tren preview, package 0.0.7.9

- Phu de co style mac dinh duoc luu trong project va hien truc tiep tren video
  khi segment dang phat co noi dung. Uu tien text dich theo cau hinh, neu trong
  thi dung text goc.
- Bam vao dong sub tren preview se mo o sua nho cua dung segment do. Bang lon
  **Subtitle style & import** chi mo khi nguoi dung chu dong bam nut nay; khong
  tu bat khi sang buoc sau STT/Translate.
- EXE portable moi:
...
- Bang chung: build release thanh cong; `TestDubbingProject` 1/1 pass va QML
  route smoke pass. Full CTest con 1 loi baseline khong lien quan caption:
  `TestColabSeparationRunner` dang doi mot cau chu notebook cu.
...
## 2026-08-24 - Recheck: reusable Voice Clone in TTS and Dubbing

- Rechecked the complete saved-clone boundary: Voice Cloning saved-reference
  library -> standalone TTS reuse -> Dubbing synthesis.
- Fixed route ownership. A verified Voice Cloning worker is now model-generic
  (not hard-wired to OmniVoice), remains independent of ordinary TTS Colab,
  and no longer changes the ordinary TTS model merely because it connects.
- A selected saved clone persists its exact Voice Cloning family with the
  Dubbing project. Selecting an ordinary built-in voice removes only clone
  state, so stale clone settings cannot affect a later normal-TTS run.
- Dubbing prefers a valid modern `savedTtsVoicePreset` over obsolete legacy
  clone keys; Direct Colab cloning requires a current explicit permission
  confirmation and verifies the exact clone worker before materializing a
  profile. A durable preset ID is part of the profile cache signature.
- Evidence: focused `TestDubbingProject`, `TestRemoteExecution`, and
  `TestColabVoiceCloneRunner` passed (3/3). Full CTest passed all 39 tests;
  the console explicitly shows 39 individual passes and zero runtime test
  failures. The stale `LastTestsFailed.log` still names an older pre-build
  `VietNormUnitTests` run, but the fresh run itself starts it as test 1 and it
  passes in 0.14 s.
- Not claimed: no owner GUI was driven, no screen capture was made, and no
  live Colab worker was used in this source-only recheck. No EXE was packaged.
